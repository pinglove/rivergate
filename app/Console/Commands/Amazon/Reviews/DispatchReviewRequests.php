...
class DispatchReviewRequests extends Command
{
    ...
    protected $description = 'Dispatch Amazon Request a Review jobs (ORDER-first; Amazon 5–30 days gate via LastUpdateDate; store trigger ASIN)';
    ...
    public function handle(): int
    {
        ...
        // Amazon window based on delivery proxy (LastUpdateDate)
        $windowStart = $now->copy()->subDays(30);
        $windowEnd   = $now->copy()->subDays(5);
        ...
        if ($debug) {
            ...
            $this->line('window_end=' . $windowEnd->toDateTimeString());
        }

        /*
         |----------------------------------------------------------------------
         | 1) Build main query (ORDER-first)
         |----------------------------------------------------------------------
         | delivery proxy = o.last_updated_date (Shipped only)
         | Amazon hard gate: 5–30 days after delivery proxy
         |----------------------------------------------------------------------
         */
        ...
        $baseQuery = DB::table('orders as o')
            ...
            ->whereNotNull('o.last_updated_date')
            ->whereBetween('o.last_updated_date', [$windowStart, $windowEnd])
            ...
            ->orderBy('o.last_updated_date')
            ->limit($limit)
            ->select([
                'o.user_id',
                'o.marketplace_id',
                'o.amazon_order_id',
                'o.last_updated_date as delivery_proxy_date',
        ...
        foreach ($rows as $row) {
            try {
                $delivery = Carbon::parse($row->delivery_proxy_date);
                ...
                if ($result === 1) {
                    ...
                    if ($debug) {
                        $this->line(
                            "queued order={$row->amazon_order_id} asin={$asin} delivery_proxy={$delivery} run_after={$runAfter}"
                        );
                    }
                } else {
                    $ignored++;
